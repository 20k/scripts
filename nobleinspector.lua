...
function noble_position(unit)
	...
	for i=0,#entity_links-1 do
		...
		imgui.Text("Fid: " .. tostring(df.global.plotinfo.group_id))
...
function basic_title_selector(unit)
	...
	local my_entity=df.historical_entity.find(df.global.plotinfo.group_id)
...
function Inspector:render()
	...
	dump_titles(df.global.plotinfo.group_id)
